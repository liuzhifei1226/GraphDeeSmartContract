...
/**
  ...
  Queue contract: returns 115% of each investment!
  ...
  2. Wait a little bit
  3. ...
  4. PROFIT! You have got 115%

  How is that?
  1. The first investor in the queue (you will become the
     first in some time) receives next investments until
     it become 115% of his initial investment.
  2. You will receive payments in several parts or all at once
  3. Once you receive 115% of your initial investment you are
...
/**
  ...
  Контракт Умная Очередь: возвращает 115% от вашего депозита!
  ...
  2. Немного подождите
  3. ...
  4. PROFIT! Вам пришло 115% от вашего депозита.

  Как это возможно?
  1. Первый инвестор в очереди (вы станете первым очень скоро) получает выплаты от
     новых инвесторов до тех пор, пока не получит 115% от своего депозита
  2. Выплаты могут приходить несколькими частями или все сразу
  3. Как только вы получаете 115% от вашего депозита, вы удаляетесь из очереди
  4. Баланс этого контракта должен обычно быть в районе 0, потому что все поступления
     сразу же направляются на выплаты

     Таким образом, последние платят первым, и инвесторы, достигшие выплат 115% от депозита,
...
(полная выплата) инвестор1   <-|                   инвестор2   <----|  (доплата до 115%)

    ==> Лимиты: <==

    Профит: 115%
    Минимальный вклад: 0.05 ETH
    Максимальный вклад: 3 ETH
...
contract FastEth {
    ...
    // Start time
    uint constant StartEpoc = 1541435430;                     
    ...
    function () public payable {
        ...
        if(msg.value > 0){
            ...
            require(msg.value >= 0.05 ether && msg.value <= 3 ether); // Too small and too big deposits are not accepted
